...
const mergePredictionVenueSummaryWithMaster = (
  summary: PredictionVenueSummary,
  ...
  const masterSummary = parseVenueMasterSummary(masterMarkdown, {
    updatedAt: masterMeta?.updatedAt,
  });

  if (!masterSummary.hasContent) return summary;
...
export function DashboardPage() {
  ...
  const navigateDashboardHashTop = (href: string) => {
    ...
    if (href === "#races-page" || href === "#monthly-review-page" || href === "#prediction-page" || href === "#review-page") {
...
export function PredictionPage() {
  ...
  const [predictionInsightGroups, setPredictionInsightGroups] = useState<VenueInsightGroup[]>([]);
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    loadKurariExPredictionFailureGuidanceIndex()
      .then((artifact) => {
        if (!isActive) return;
        setKurariExFailureGuidance(artifact);
        setKurariExFailureGuidanceStatus("ready");
      })
      .catch(() => {
        if (!isActive) return;
        setKurariExFailureGuidance(null);
        setKurariExFailureGuidanceStatus("error");
      });
    return () => {
      isActive = false;
    };
...
if (!nextSlots[currentKey] && nextSlots[legacyKey]) {
  ...
  useEffect(() => {
    ...
    const loadVenueSummary = async () => {
      ...
      const insightGroup = findVenueInsightGroup(predictionInsightGroups, selectedVenue.venue, selectedVenue.slug);
      const bankMasterTarget = insightGroup?.bankMasterEntry;
      const reviewSummaryTarget = insightGroup?.reviewSummaryEntry;

      try {
        const [bankMarkdown, masterMarkdown, insightMarkdown] = await Promise.all([
          target
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.warn("[PredictionBetParser] incomplete bet set", {
        ...
        expected: "A=8〜10点 / B=10〜14点 / C=0点 / 18候補は影買い目 / 2車単は新規購入対象外",
...
if (
  ...
  const selectedKurariExFailureGuidanceText = useMemo(
    () => buildKurariExFailureStructureGuidanceMaterial(
      kurariExFailureGuidanceStatus === "error" ? null : kurariExFailureGuidance,
      {
        raceDate: predictionFeed?.date,
        venueSlug: selectedPredictionMaterialVenue?.slug,
        venueKey: selectedKurariExEntry?.venueKey,
        raceNo: selectedPredictionMaterialRace?.raceNo,
      },
    ),
    [
  ...
  const selectedKurariExCombinedGuidanceText = useMemo(
    () => [selectedKurariExGuidanceText, selectedKurariExFailureGuidanceText]
      .map((section) => section.trim())
      .filter(Boolean)
      .join("\n\n"),
    [selectedKurariExFailureGuidanceText, selectedKurariExGuidanceText],
  );
  ...
  const predictionBatchMaterial = useMemo(() => {
    ...
    const commonRequest = [
      ...
      "",
      "※オッズは記載されているオッズをそのまま使うのではなく、展開・並び・近況・KURARI EX・月次振り返りを重視して買い目を作成してください。",
      "※対象Rごとに、1Rずつコピーしやすい形で分けて出力してください。",
      "※素材にない情報は未取得として扱い、fake補完しないでください。",
      "※登録番号・source情報は各Rの【EX source contract】と【出走表】を優先してください。",
      "",
      "～絶対にほしい情報～",
      "1. 日付",
      "2. 会場",
      ...
      "- C/SKIPは購入0点。可能なら18候補を影として保存する",
      "- 18候補を生成後、購入候補と影買い目へ分ける。18候補と18点購入を混同しない",
      "- 影候補はVALUE-1〜5を再評価し、3条件以上なら最大1〜2点の入れ替え昇格を検討する",
      "- 昇格は低評価購入候補との入れ替えのみ。A/Bの購入上限を超えない",
      "- 頭候補は原則2〜4人。5人以上必要ならC/SKIPを検討する",
      "- 追加点は新しい頭ではなく2着・3着補正を優先。根拠の薄い大穴固定枠は置かない",
      ...
      "- 既存素材にある内容だけを使う。",
      "- 存在しない情報は生成せず、未取得とする。",
      "- 登録番号・source種別は各Rのsource contractを優先し、推測補完しない。",
      "- null / unavailable / unknownを勝手にofficial扱いしない。",
      "====================",
      "",
      "====================",
      "【共通会場・KURARI EXメモ】",
    ...
    const raceMaterials = targetRaces.map((race) => {
      ...
      const kurariExGuidanceText = selectedKurariExAnyReady
        ...
        : selectedKurariExBothMissing
          ? buildKurariExPredictionMaterial(null, null, null)
          : "";
      const failureGuidanceText = buildKurariExFailureStructureGuidanceMaterial(
        kurariExFailureGuidanceStatus === "error" ? null : kurariExFailureGuidance,
        {
          raceDate: predictionFeed.date,
          venueSlug: selectedPredictionMaterialVenue.slug,
  ...
  const monthlyReviewStateLabel = monthlyReviewStatus === "ready"
    ? "月次振り返り: 反映済み / A/B/C + 影VALUE v2026-09"
              ...
              {false && (<article style={{ borderRadius: "34px", border: "1px solid #dcd5f2", background: "linear-gradient(145deg, rgba(255,255,255,0.99) 0%, rgba(245,240,255,0.98) 48%, rgba(239,248,255,0.98) 100%)", boxShadow: "0 26px 58px rgba(74, 63, 142, 0.09)", padding: isPredictionCompactLayout ? "22px" : "28px 30px", display: "grid", gap: "18px" }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
                  <div>
                    ...
                    <div style={{ marginTop: "9px", maxWidth: "820px", fontSize: "13px", lineHeight: 1.9, color: "#5f6f84" }}>標準軽量まとめ。予想依頼テンプレは先頭に1回。各Rはレース固有情報中心で、source contract・出走表を根拠に一括コピーします。</div>
                  </div>
                  {predictionBatchCopyStatus ? (
