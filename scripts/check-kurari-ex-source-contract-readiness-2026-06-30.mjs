...
import {
  auditKurariExSourceContractReadiness20260630,
} from "./audit-kurari-ex-source-contract-readiness-2026-06-30.mjs";
...
export async function checkKurariExSourceContractReadiness20260630({
  printOutput = true,
} = {}) {
  const failures = [];
  const warnings = [];
  ...
  const indexBuffer = await readFile(abs(indexPath));
  ...
  const historyBuffer = await readFile(abs(historyPath));
  if (
  ...
  const audit = await auditKurariExSourceContractReadiness20260630({
    printOutput: false,
  });
  const summary = audit.sourceContractReadinessSummary;
  if (
    summary.finalStatus
      !== "SOURCE_CONTRACT_READINESS_AUDIT_COMPLETED_WITH_WARNINGS"
  ) failures.push(`source-contract audit failed: ${summary.finalStatus}`);
  if (
    ...
    || !summary.authoritativeSourceCollectionRequired
    || summary.predictionFileCount !== 8
    || summary.summaryFileCount !== 8
    || summary.resultFileCount !== 8
    || summary.predictionStructuredBlockCount !== 0
    || summary.summaryStructuredBlockCount !== 0
    || summary.resultStructuredBlockCount !== 0
    || summary.predictionContractReady
    || summary.summaryContractReady
    || summary.resultOutputContractReady
    || summary.canProceedToBackfillDryRun
    || summary.canProceedToBackfillWrite
    || summary.writePerformed
  ) failures.push("source-contract readiness baseline changed");
  ...
  if (status.status !== 0) {
    ...
    for (const file of changed) {
      ...
      failures.push(`changed file is outside this source-contract task: ${file}`);
    }
  ...
  const finalStatus = failures.length
    ? "SOURCE_CONTRACT_READINESS_CHECK_FAIL"
    : warnings.length
      ? "SOURCE_CONTRACT_READINESS_CHECK_PASS_WITH_WARNINGS"
      : "SOURCE_CONTRACT_READINESS_CHECK_PASS";
...
if (isMain) await checkKurariExSourceContractReadiness20260630();
